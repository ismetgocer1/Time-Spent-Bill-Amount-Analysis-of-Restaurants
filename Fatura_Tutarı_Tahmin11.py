...
# Kullanıcı verilerini modelin beklediği formata dönüştürme
df_user_encoded = pd.get_dummies(df_user, columns=['Live Music', 'Reservation', 'Meal Type', 'Day', 'Gender', 'Table Location', 'Age Group', 'Weather Condition'], drop_first=True)
for column in df_user_encoded.columns:
    if df_user_encoded[column].dtype == 'bool':
        df_user_encoded[column] = df_user_encoded[column].astype(int)
df_user_encoded = df_user_encoded.reindex(columns=encoded_data.columns, fill_value=0)
...
df_user_encoded = df_user_encoded.reindex(columns=columns_expected_by_model, fill_value=0)

# Verileri ölçeklendirme
df_user_scaled = scaler.transform(df_user_encoded)
